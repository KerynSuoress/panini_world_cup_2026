...
export const POST: APIRoute = async ({ params, request }) => {
  ...
  try {
    ...
    if (trade.initiatorId !== profileId) {
      return new Response(JSON.stringify({ error: "Only the initiator can send a reminder" }), {
        status: 403,
        headers,
      });
    }
