...
export const POST: APIRoute = async ({ params, request }) => {
  ...
  try {
    ...
    if (trade.partnerId !== profileId) {
      return new Response(JSON.stringify({ error: "Only the partner can decline this trade" }), {
        status: 403,
        headers,
      });
    }
    ...
    await db
      ...
      .set({ status: "declined", resolvedAt: new Date(), reminderPending: false })
      .where(and(eq(tradeRequests.id, tradeId), eq(tradeRequests.status, "pending")));

    return new Response(JSON.stringify({ ok: true }), { headers });
  } catch (err) {
    console.error("[trade-requests decline] DB error:", err);
